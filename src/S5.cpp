...
double gn(double r, object obj, double g = 1)
{
	switch (obj)
	{
		case HARD:
		...
		case SUN:
			if (r < r_sun)	return g * Solar_density(r); // r in cm, density is in g/cc
			else			return 0;
		case EARTH:
			if (r < r_earth)	return g * Earth_density(r); // r in cm, density is in g/cc
			else				return 0;
		default:
...
struct diff_eq_params {double m, lambda, phi0, g; object obj;};
int diff_eq(double r, const double y[], double f[], void *params)
{
	...
	double X;
	if (r < 1e-5 * r_s(p.obj))
		X = (gn(0, p.obj, p.g) / 3) + (p.lambda / 18) * cube(p.phi0) + (sq(p.m) / 3) * p.phi0;
	else
		X = y[1] / r;
	f[1] = gn(r, p.obj, p.g) - 2. * X + sq(p.m) * y[0] + p.lambda * cube(y[0]) / 6;
	return GSL_SUCCESS;
}
...
double phi0_Guess(double m, double lambda, double g, object obj)
{
	...
	if (m * radius < 1e-5)
		A = 0.5 * sq(radius) * gn(0, obj, g);
	else
		A = gn(0, obj, g) * (1 - exp(-m * radius) * (1 + m * radius)) / sq(m);
	if (lambda == 0) return -A;

	// B: the large lambda limit
	B = pow(6 * gn(0, obj, g) / lambda, 1. / 3);

	return -pow(pow(A, -alpha) + pow(B, -alpha), -1. / alpha);
}

// helper function and parameters for the differential equation solver
struct phi0_KG_helper_params {double lambda, phi0_guess, m, g; object obj;};
double phi0_KG_helper(double phi0, void *params)
{
	...
	diff_eq_params p2 = {p.m, p.lambda, phi0, p.g, p.obj};
...
double phi0_KG(double m, double lambda, double g, object obj)
{
	...
	double phi0_guess = phi0_Guess(m, lambda, g, obj);
	gsl_root_fsolver *s;
	phi0_KG_helper_params p = {lambda, phi0_guess, m, g, obj};
...
{
	diff_eq_params p = {m, lambda, phi0, g, obj};
